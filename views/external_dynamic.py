...
class ExternalDynamicContent(BaseContent):
    ...
    def __init__(self, master):
        ...
        # Configuration row
        cfg_row = 0
        ctk.CTkLabel(top_frame, text="Cubetas:").grid(row=cfg_row, column=0, padx=(8,4), pady=6, sticky='w')
        self.entry_cols = ctk.CTkEntry(top_frame, width=80)
        self.entry_cols.grid(row=cfg_row, column=1, padx=4, pady=6)

        ctk.CTkLabel(top_frame, text="Registros:").grid(row=cfg_row, column=2, padx=(12,4), pady=6, sticky='w')
        self.entry_records = ctk.CTkEntry(top_frame, width=80)
        self.entry_records.grid(row=cfg_row, column=3, padx=4, pady=6)

        ctk.CTkLabel(top_frame, text="DO expansión:").grid(row=cfg_row, column=4, padx=(12,4), pady=6, sticky='w')
        self.entry_do = ctk.CTkEntry(top_frame, width=80)
        self.entry_do.grid(row=cfg_row, column=5, padx=4, pady=6)

        ctk.CTkLabel(top_frame, text="DO reducción:").grid(row=cfg_row, column=6, padx=(12,4), pady=6, sticky='w')
        ...
        # Operations row
        ops_row = 1
        ctk.CTkLabel(top_frame, text="Clave:").grid(row=ops_row, column=0, padx=(8,4), pady=(0,8), sticky='w')
        self.key_entry = ctk.CTkEntry(top_frame, width=120)
        self.key_entry.grid(row=ops_row, column=1, padx=4, pady=(0,8))

        self.insert_btn = ctk.CTkButton(top_frame, text="Insertar", command=self.on_insert)
        self.insert_btn.grid(row=ops_row, column=2, padx=(8,4), pady=(0,8))

        self.delete_btn = ctk.CTkButton(top_frame, text="Eliminar", command=self.on_delete)
        self.delete_btn.grid(row=ops_row, column=3, padx=(4,4), pady=(0,8))

        self.search_btn = ctk.CTkButton(top_frame, text="Buscar", command=self.on_search)
        self.search_btn.grid(row=ops_row, column=4, padx=(8,4), pady=(0,8))

        self.save_btn = ctk.CTkButton(top_frame, text="Guardar", command=self.on_save)
        self.save_btn.grid(row=ops_row, column=5, padx=(8,4), pady=(0,8))

        self.load_btn = ctk.CTkButton(top_frame, text="Cargar", command=self.on_load)
        self.load_btn.grid(row=ops_row, column=6, padx=(4,4), pady=(0,8))

        self.clear_btn = ctk.CTkButton(top_frame, text="Limpiar", fg_color="#b00020", hover_color="#c62828", command=self.on_clear)
    ...
    def on_insert(self):
        ...
        try:
            ...
            msg = f"Insertado {key}. DO={res['do']:.2f}%"
            self.status.configure(text=msg)
            try:
                self.key_entry.delete(0, 'end')
            except Exception:
